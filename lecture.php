    ...
    <script>
        ...
        function Onchange(ajax){
            ...
            new Ajax.Request("get_qna.php", {
                method: "get",
                parameters: {"lecture_id": lecture_id},
                onSuccess: get_qna,
            ...
            for (i = 0; i < data.length; i++) {
                ...
                for(j = 0; j < answers.length; j++){
                    if (answers[j]['q_id'] == data[i]['id']){
                        ...
                        var who = $(document.createElement('p'));
                        who.addClassName("who");
                        who.innerHTML = answers[j]['name'] + '<span class="right-align" onclick="a_del('+ answers[j]['id'] + ','+ answers[j]['password']+')"><a class="btn fas fa-minus"></a></span>';
                        var when = $(document.createElement('p'));
                        when.addClassName("when");
